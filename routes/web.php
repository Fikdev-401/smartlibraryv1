...
Route::group(['middleware'=>['auth:user','ceklevel:SUPERUSER,ADMIN,OPERATOR']], function(){
	...
	Route::get('/gen/ebook/detail/{id}', 'Admin\EbookCont@detail')->name('gen.ebook.detail');
	...
	Route::get('/gen/user/upload/details/ajax/{id}', 'Admin\UserUploadController@ajax')->name('user.details.upload');
	...
	// ============================================================
	// SISTEM BUKU FISIK & PEMINJAMAN (Operator - Buku Fisik)
	// ============================================================
	Route::get('/op/buku-fisik','Admin\BukuFisikCont@index')->name('op.buku-fisik');
	Route::get('/op/buku-fisik/create','Admin\BukuFisikCont@create')->name('op.buku-fisik.insert');
	Route::post('/op/buku-fisik/post','Admin\BukuFisikCont@store')->name('op.buku-fisik.post');
	Route::get('/op/buku-fisik/edit/{id}','Admin\BukuFisikCont@edit')->name('op.buku-fisik.edit');
	Route::post('/op/buku-fisik/update','Admin\BukuFisikCont@update')->name('op.buku-fisik.update');
	Route::get('/op/buku-fisik/delete/{id}','Admin\BukuFisikCont@destroy')->name('op.buku-fisik.delete');
	Route::get('/op/buku-fisik/detail/{id}','Admin\BukuFisikCont@detail')->name('op.buku-fisik.detail');

	Route::get('/op/peminjaman','Admin\PeminjamanCont@index')->name('op.peminjaman');
	Route::get('/op/peminjaman/create','Admin\PeminjamanCont@create')->name('op.peminjaman.insert');
	Route::post('/op/peminjaman/post','Admin\PeminjamanCont@store')->name('op.peminjaman.post');
	Route::get('/op/peminjaman/detail/{id}','Admin\PeminjamanCont@show')->name('op.peminjaman.detail');
	Route::get('/op/peminjaman/kembali/{id}','Admin\PeminjamanCont@kembali')->name('op.peminjaman.kembali');
	Route::post('/op/peminjaman/kembali/post','Admin\PeminjamanCont@kembaliStore')->name('op.peminjaman.kembali.post');
	Route::get('/op/peminjaman/terlambat','Admin\PeminjamanCont@terlambat')->name('op.peminjaman.terlambat');
