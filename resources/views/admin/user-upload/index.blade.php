                ...
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Daftar Upload</h5>
                        </div>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item">Daftar Upload e-Book per User</li>
                        </ul>
                    </div>
                </div>
            </div>
                        ...
                        <table id="res-config" class="table table-striped table-hover dt-responsive" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <th>User</th>
                                    <th>Email</th>
                                    <th width="1%" class="text-center">Jumlah Upload</th>
                                    <th width="1%" data-ordering="false">&nbsp</th>
                                </tr>
                                <?php $no=1; ?>
                            </thead>
                            <tbody>
                                @foreach($user as $r)
                                <?php $qty=UserUpload::getJmlPerUser($r->id); ?>
                                <tr>
                                    <td><?=$no?></td>
                                    <td>{{$r->name}}</td>
                                    <td>{{$r->email}}</td>
                                    ...
                                    <td class="text-center">
                                        <a href="{{route('gen.user.upload.details',['id'=>$r->id])}}" class="btn btn-sm btn-warning"><i data-feather="eye"></i> Details</a>
                                    </td>
                                </tr>
                                <?php $no++?>
                                @endforeach
